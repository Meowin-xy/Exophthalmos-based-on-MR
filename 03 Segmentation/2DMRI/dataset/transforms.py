...
class ToTensor:
    ...
    def __call__(self, sample):
        ...
        # Only process 'target' if it exists and mode is 'train'
        if self.mode == 'train' and 'target' in sample:
            mask = sample['target']
...
class NormalizeIntensity:
    ...
    def __call__(self, sample):
        img = sample['input']
        img[:, :, :, 0] = self.normalize_zscore(img[:, :, :, 0])
        # img[:, :, :, 1] = self.normalize_pt(img[:, :, :, 1])
        # img[:, :, :, 0] = self.normalize_pt(img[:, :, :, 0])
        sample['input'] = img
        return sample

    @staticmethod
    def normalize_ct(img):
        norm_img = np.clip(img, -1024, 1024) / 1024   # ct 归一化[-1, 1]
        # norm_img = np.clip(img, -1000, 1000) / 1000  # ct 归一化[-1, 1]
        return norm_img
