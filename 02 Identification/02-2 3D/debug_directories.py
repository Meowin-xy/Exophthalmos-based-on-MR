...
# 预测单个文件并可视化
def predict_single_file(model, file_path, prob_threshold=0.5):
    """
    预测单个文件并可视化结果
    """
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    try:
        ...
        # 转换为PyTorch张量
        img_tensor = torch.from_numpy(img_slice).float().unsqueeze(0).unsqueeze(0)  # 添加批次和通道维度
        ...
        with torch.no_grad():
            ...
            color_mask[threshold_mask > 0] = [0.0, 0.0, 1.0, 0.3]  # 蓝色，透明度0.3
            axes[0].imshow(color_mask)
            axes[0].set_title(f'Original Image + Threshold Regions (>{prob_threshold})')
            axes[0].axis('off')
            
            # 第二个子图：概率热图
            im = axes[1].imshow(pred_np, cmap='jet', vmin=0, vmax=1)
            fig.colorbar(im, ax=axes[1], fraction=0.046, pad=0.04)
            axes[1].set_title('Prediction Probability Map')
            axes[1].axis('off')
            ...
            # 标记预测点
            pred_points = []
            if upper_pred_y is not None:
                axes[2].scatter(upper_pred_x, upper_pred_y, c='r', marker='x', s=50, label=f'Upper Centroid ({upper_pred_y}, {upper_pred_x})')
                pred_points.append(f'Upper: ({upper_pred_y}, {upper_pred_x})')
            
            if lower_pred_y is not None:
                axes[2].scatter(lower_pred_x, lower_pred_y, c='g', marker='x', s=50, label=f'Lower Centroid ({lower_pred_y}, {lower_pred_x})')
                pred_points.append(f'Lower: ({lower_pred_y}, {lower_pred_x})')
            
            axes[2].set_title('Image + Threshold Regions + Predicted Centroids')
            axes[2].legend(loc='upper right')
            axes[2].axis('off')
            
            plt.suptitle(f'Prediction Results for: {os.path.basename(file_path)}', fontsize=14)
            plt.tight_layout()
            plt.show()
...
def main():
    ...
    # 指定使用特定模型文件
    model_filename = "zygomatic_model_20250625_155114.pth"
    ...
    # 指定要预测的单个文件
    target_file = r"C:\Users\xrVis001\Desktop\EyeData\Eyeball0714\Eyeball0714\Validation\3D\quantuSlice\slice_83310521-20230603.nii.gz"
    
    # 预测单个文件并可视化
    print(f"\n预测指定文件: {target_file}")
    predict_single_file(model, target_file, prob_threshold=prob_threshold)
